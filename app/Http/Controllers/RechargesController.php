        ...
        try {
            ...
            if ($apiResponse['STATUS'] == 3) { // Failure case
                ...
                return response()->json([
                    'apiResponse' => [
                        'STATUS' => $apiResponse['STATUS'] ?? 3,
                        'MESSAGE' => $apiResponse['MESSAGE'] ?? 'Request failed!',
                        'ERROR_MESSAGE' => $apiResponse['ERROR_MESSAGE'] ?? 'Unknown error', // Correct key name
                        'ERRORCODE' => $apiResponse['ERRORCODE'] ?? '38',
                        'REQUESTTXNID' => $apiResponse['REQUESTTXNID'] ?? 'txn_unknown',
                        'HTTPCODE' => $apiResponse['HTTPCODE'] ?? 200 // Ensure this matches if necessary
                    ]
